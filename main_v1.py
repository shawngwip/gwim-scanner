    ...
    if event.name == "enter":
        ...
        if is_reset_code(barcode):
            ...
            if not is_valid_staff_id(normalized_barcode):
                ...
                debug(f"Valid staff ID detected: {normalized_barcode}")
                ...
                try:
                    ...
                    debug(f"🔍 Fetching staff info for ID: {staff_id}")
                    cursor.execute("""
                    ...
                    if not staff_row:
                        debug(f"❌ Staff ID not eligible: {staff_id}")
                        blink_light(RED_PIN, times=1)
                        buzz(times=1)
                        return

                    # ✅ Step 1: Refresh allocation_temp_test
                    cursor.execute("SELECT * FROM allocation_temp_test WHERE staffid = %s", (staff_id,))
                    existing = cursor.fetchone()

                    if existing:
                        debug("↩️ Deleting old allocation_temp_test entry...")
                        cursor.execute("DELETE FROM allocation_temp_test WHERE staffid = %s", (staff_id,))

                    debug("➕ Inserting new allocation_temp_test entry...")
                    cursor.execute("""
                    ...
                    """, (
                        ...
                        staff_row['staffdept'], datetime.now().date(), staff_row['pic']
                    ))

                    # ✅ Step 2: Update/Insert allcation_log_test
                    today_str = datetime.now().strftime('%Y-%m-%d')
                    cursor.execute("""
                        SELECT * FROM allcation_log_test
                        WHERE employee_id = %s AND date_run = %s
                    """, (staff_id, today_str))
                    log_row = cursor.fetchone()

                    if log_row:
                        debug("✏️ Updating allcation_log_test (OUT)...")
                        cursor.execute("""
                            UPDATE allcation_log_test
                            SET out_datetime = %s, status = 'OUT'
                            WHERE id = %s
                        """, (datetime.now(), log_row['id']))
                    else:
                        debug("🆕 Inserting new allcation_log_test (IN)...")
                        ...
                        """, (
                            ...
                            staff_row['staffdept'], datetime.now(), staff_row['pic'],
                            today_str, datetime.now(), shift_value
                        ))

                    # ✅ Step 3: Update/Insert prod_attendance_test
                    cursor.execute("""
                        SELECT * FROM prod_attendance_test
                        WHERE staffid = %s AND date = %s
                    """, (staff_id, today_str))
                    attendance_row = cursor.fetchone()

                    if attendance_row:
                        debug("✏️ Updating prod_attendance_test (timeout)...")
                        cursor.execute("""
                            UPDATE prod_attendance_test
                            SET timeout = %s
                            WHERE id = %s
                        """, (datetime.now(), attendance_row['id']))
                    else:
                        debug("🆕 Inserting new prod_attendance_test...")
                        shift = staff_row.get('shift', '').upper()
                        shift_value = 'DAY' if 'DAY' in shift else 'NIGHT' if 'NIGHT' in shift else ''
                        cursor.execute("""
                        ...
                        """, (
                            ...
                            datetime.now(), staff_row['pic'], today_str, shift_value,
                            staff_row.get('staffagency', ''), calendar.day_name[datetime.today().weekday()]
                        ))

                    conn.commit()
                    conn.close()

                    # ✅ Indicate success via hardware
                    blink_light(GREEN_PIN, times=1)
                    buzz(times=1)

                except Exception as e:
                    debug(f"🔥 Staff DB operation failed: {e}")
                    blink_light(RED_PIN, times=1)
                    buzz(times=1)
                    return
